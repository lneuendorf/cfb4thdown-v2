...
flowchart TB
  ...
  subgraph railway["Railway: one Docker service"]
    ...
    api["FastAPI · /api/v1<br/>scoreboard · games · teams · ticker · health"]
    db[("SQLite on /data volume<br/>games · plays_fourth_down · wp_series<br/>game_sources · pregame_snapshots · teams · run_log")]
  end

  subgraph vercel["Vercel"]
    web["React + Vite site<br/>home (latest week) · game page · games · methodology"]
  end
...
Phases 0–2 are built and run locally: models, grading, the 2013–2025 backfill, current-season processing, the API, scheduled jobs, and the home, game and games pages. Nothing is deployed yet; follow `docs/deploy.md`. See `docs/roadmap.md`.
